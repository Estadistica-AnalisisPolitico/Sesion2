...
Regresión Lineal Multivariada (II)
...
Profesores de la Universidad de los Andes [@mejiaguinand2008a] decidieron estudiar cómo la distribución de fondos públicos fue afectada por factores políticos durante el primer periodo del Presidente Uribe (2002-2006). Las hipótesis que se plantean son:
...
Recuerda además que es mejor usar datos sin valores perdidos en los análisis multivariados ^[Esto puede significar que encuentres resultados distintos a los del trabajo original.].
...
El resultado anterior puede ser presentado de mejor manera usando **modelsummary** package, como se puede ver en la Tabla
\@ref(tab:prettyReg1).
...
Esto es información suficiente para representar esa relación con la 
Ecuación \@ref(eq:reg1).
...
# modelsummary(tanova,)
kable(tanova,
      caption = "This is a table")%>%kableExtra::kable_styling(full_width = FALSE)
```
...
En el caso actual, no hemos hecho ello, pues las variables son de diverso tipo. Así, si añadimos la variable **uribista** al último modelo (Tabla \@ref(tab:reg2)), obtenemos la regresión de la Tabla \@ref(tab:reg3).
...
Los residuos, la diferencia entre *apropiaciondolar* y $\hat{apropiaciondolar}$, deben distribuirse de manera normal:
